...
Generally, we can implement the extrapolation as: $\text{PredictDistance} = \text{PrevDistance} + \frac{(\text{Current ToF DistReading}) - (\text{Previous To DistReading})}{(\text{Time Gap between the Last Two ToF Distance Readings})} \times \\
(\text{Time Gap of the Last Two Time Readings from PID Loop})$
