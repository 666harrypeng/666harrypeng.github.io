...
Finally, I hope to optimize the KF parameters' values to fit the robot car's motion. I tested the KF with ($\sigma_1$, $\sigma_2$, $\sigma_3$) = (2, 1, 0.02), which indicated that we trust ***less on our model*** and ***more on the measurement***.
