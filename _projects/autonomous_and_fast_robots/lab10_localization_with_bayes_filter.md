...
toc: true
redirect_from:
  - /projects/autonomous-and-fast-robots-cornell/lab10-localization-with-bayes-filter/
---
...
Here, we simulate our robot on the grid. The robot state consists of three parts - $(x, y, \theta)$, and the robot's world is a continuous space. (with the spans: $x \in [-1.6764, +1.9812)\text{meters}$, $y \in [-1.3716, +1.3716) \text{meter}$, $\theta \in [-180, +180) \text{degrees}$)
...
![compute_control_code](../attachments/lab10/compute_control_code.png "compute_control_code")
...
![odom_motion_model](../attachments/lab10/odom_motion_model.png "odom_motion_model")
...
![prediction_step](../attachments/lab10/prediction_step.png "prediction_step")

### Task 4 - Sensor Model

Here, we use the Gaussian model to calculate the probability of each sensor's measurement.

![sensor_model](../attachments/lab10/sensor_model.png "sensor_model")
...
![update_step](../attachments/lab10/update_step.png "update_step")
